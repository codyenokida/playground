...
export default function Div({ id, initialExpand = false, children }: H2Props) {
  ...
  return (
    <>
      ...
      >
        <span className="text-red">{`<div `}</span>
        <span className="text-orange">{`id`}</span>
        <span className="text-white">{`="`}</span>
        <span className="text-green">{`${id}`}</span>
        <span className="text-white">{`" `}</span>
        <span className="text-orange">{`onClick`}</span>
        <span className="text-white">{`="`}</span>
        <span className="text-green">{`${expanded ? `hide()` : `expand()`}`}</span>
        <span className="text-white">{`"`}</span>
        <span className="text-red">{`>`}</span>
        <span className="text-gray">{`${expanded ? "" : "..."}`}</span>
        <span className="text-red">{`${expanded ? "" : "</div>"}`}</span>
      </button>
      <motion.div
        animate={{
          x: expanded ? 0 : -20,
